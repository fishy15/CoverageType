...
let[@axiom] tree_ch_depth_minus_1 (t : int tree) (l : int tree) (r : int tree) =
  (lch t l && rch t r)#==>((depth t == depth l + 1 || depth t == depth r + 1) && depth t > depth l && depth t > depth r)

let[@axiom] tree_leaf_cnt_unique (t : int tree) (x : int) (y : int) =
...
let[@axiom] tree_no_mem_leaf (t : int tree) =
  iff (fun (x : int) -> not (tree_mem t x)) (depth t == 0)

let[@axiom] tree_single_value (t : int tree) (x : int) =
  (depth t == 1 && root t x)#==>(fun (y : int) -> (tree_mem t y)#==>(x == y))

let[@axiom] tree_unique_leaf (t1 : int tree) (t2 : int tree) =
...
let[@axiom] tree_unique_lch (t : int tree) (l1 : int tree) (l2 : int tree) =
  (lch t l1 && lch t l2)#==>(l1 == l2)
...
let[@axiom] tree_depth_0_no_mem (t : 'a tree) (x : int) =
...
let[@axiom] eqv_spine_eq_depth =
  fun (l1 : 'a tree) (l2 : 'a tree) -> 
    (eqv_spine l1 l2)#==>(depth l1 == depth l2)
