...
let rec sub_rty rctx (rty1, rty2) =
  ...
  let aux rctx (rty1, rty2) =
    ...
    | ( RtyBase { ou = Over; cty = cty1; eqv = None },
        RtyBase { ou = Over; cty = cty2; eqv = None } ) ->
        if equal_cty (fun _ _ -> true) cty1 cty2 then true
        else sub_cty Over rctx cty1 cty2
    | ( RtyBase { ou = Under; cty = cty1; eqv = None },
        RtyBase { ou = Under; cty = cty2; eqv = None } ) ->
        if equal_cty (fun _ _ -> true) cty1 cty2 then true
...
let non_emptiness_rty rctx rty =
  let () = Statistic.stat_count_qeury rctx.task_name in
  match rty with
  | RtyBase { ou = Under; cty; eqv = None } -> non_emptiness_cty rctx cty
  | RtyArr _ -> true
  | RtyPolyPred _ -> true
